...
| 函数名 | 功能概述 | 参数 | 返回值 | 备注 |
| --- | --- | --- | --- | --- |
| initRobot(const std::string & local_ip, const int local_port, const string& dog_ip = "192.168.234.1") | 与机器狗建立通信，机器狗端ip默认“192.168.234.1”，如更改机器人ip，需从该接口传入新ip | 用户主机ip和端口 | 无 | 通信建立失败时，终端输出失败日志 |
| lieDown() | 控制机器狗站低 | 无 | 进入状态正常返回0，返回0x3012代表电机数据丢失，0x3010代表电机失能，0x3011代表电机故障，0x3009代表电机角度超限，0x3007代表状态机切换失败，0x3013代表速度命令过大 | 机器狗站低，关节锁定 |
| standUP() | 控制机器狗站立 | 无 | 进入状态正常返回0，返回0x3012代表电机数据丢失，0x3010代表电机失能，0x3011代表电机故障，0x3009代表电机角度超限，0x3007代表状态机切换失败，0x3013代表速度命令过大 | 机器狗正常站立，关节锁定 |
| passive() | 控制机器狗紧急趴下 | 无 | 进入状态正常返回0，返回0x3012代表电机数据丢失，0x3010代表电机失能，0x3011代表电机故障，0x3009代表电机角度超限，0x3007代表状态机切换失败，0x3013代表速度命令过大 | 机器狗进入阻尼状态，趴下 |
| move(float vx, float vy, float yaw_rate) | 控制机器狗运动 | 前向速度、侧向速度、绕Z轴角速度 | 进入状态正常返回0，返回0x3012代表电机数据丢失，0x3010代表电机失能，0x3011代表电机故障，0x3009代表电机角度超限，0x3007代表状态机切换失败，0x3013代表速度命令过大 | 机器狗站立状态下切入，控制狗按指定速度运动。vx范围（-3m/s~-0.4m/s；0.4m/s~3m/s），vy范围（-2.0m/s~-0.2m/s；0.2m/s~2.0m/s），yaw_rate范围（-2.0rad/s-2.0rad/s） |
| jump() | 控制机器狗垂直跳跃 | 无 | 进入状态正常返回0，返回0x3012代表电机数据丢失，0x3010代表电机失能，0x3011代表电机故障，0x3009代表电机角度超限，0x3007代表状态机切换失败，0x3013代表速度命令过大 | 垂直跳高，基于站立状态下切入 |
| frontJump() | 控制机器狗向前跳 | 无 | 进入状态正常返回0，返回0x3012代表电机数据丢失，0x3010代表电机失能，0x3011代表电机故障，0x3009代表电机角度超限，0x3007代表状态机切换失败，0x3013代表速度命令过大 | 站立状态下切入 |
| backflip() | 控制机器狗后空翻 | 无 | 进入状态正常返回0，返回0x3012代表电机数据丢失，0x3010代表电机失能，0x3011代表电机故障，0x3009代表电机角度超限，0x3007代表状态机切换失败，0x3013代表速度命令过大 | 站立状态下切入 |
| attitudeControl(float yaw_vel, float roll_vel, float pitch_vel, float height_vel) | 控制机器狗原地扭动和身体高度变化 | 无 | 进入状态正常返回0，返回0x3012代表电机数据丢失，0x3010代表电机失能，0x3011代表电机故障，0x3009代表电机角度超限，0x3007代表状态机切换失败，0x3013代表速度命令过大 | 机器狗站立状态下切入，控制狗按指定速度原地扭动，身体高度变化。yaw_vel, pitch_vel, roll_vel范围（-0.5rad/s~0.5rad/s），height_vel范围（-0.5m/s-0.5m/s） |
| getRoll() | 获取机器狗当前翻滚角 | 无 | 返回当前翻滚角度 | 单位弧度 |
| getPitch() | 获取机器狗当前俯仰角 | 无 | 返回当前俯仰角度 | 单位弧度 |
